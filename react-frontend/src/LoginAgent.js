...
function LoginAgent() {
  ...
  // Resetting user id when login is loaded
  localStorage.clear();
  ...
  async function handleSubmit(event) {
    ...
    if (_response["status"] == 200) {
      ...
      setEmail("");
      setIsPending(false);
      const currentUserId = _response["data"]["user_id"];
  ...
  return (
    <div>
      <Form onSubmit={handleSubmit}>
        ...
        <Form.Group
          className="mb-3 styleform small"
          controlId="formBasicCheckbox"
        >
          <Form.Check type="checkbox" label="Remeber me" />
        </Form.Group>
        {!isPending && (
          <button className="d-block btn-primary btn-user w-100" type="submit">
            Login
          </button>
        )}
        {isPending && (
          <button className="d-block btn-primary btn-user w-100" disabled>
            Processing...
          </button>
        )}
      </Form>
